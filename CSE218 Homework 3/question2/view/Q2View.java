...
public class Q2View {
	...
	public Q2View() {
		...
		push.setOnAction(e->{
			...
			Q2Controller.processPush(newName, newGpa);
			studentOutput.appendText(Q2Controller.updateTextAreaPush());
		});
		Button pop = new Button("Stack Pop");
		pop.setOnAction(e->{
			Q2Controller.processPop();
			studentOutput.appendText(Q2Controller.updateTextAreaPop());
		});
		Button insert = new Button("Queue Insert");
		insert.setOnAction(e->{
			String newName = name.getText();
			String newGpa = gpa.getText();
			Q2Controller.processInsert(newName, newGpa);
			studentOutput.appendText(Q2Controller.updateTextAreaInsert());
		});
		Button remove = new Button("Queue Remove");
		remove.setOnAction(e->{
			Q2Controller.processRemove();
			studentOutput.appendText(Q2Controller.updateTextAreaRemove());
		});
		grid.add(name, 0, 0);
